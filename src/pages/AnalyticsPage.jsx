...
const OUTCOME_COLORS = {
  ...
  meeting_booked: '#3b9eff', voicemail_left: '#9999b0',
  wrong_number: '#3a3a50', busy: '#6c63ff',
};
...
export default function AnalyticsPage() {
  ...
  // Admin with filter: the selected caller UUID
  // Employee: their own UUID, or '__PENDING__' if not loaded yet
  let callerId;
  if (isAdmin) {
    callerId = filterCaller || 'ALL';
  } else {
    callerId = caller?.id || '__PENDING__';
  }

  // Load callers dropdown (admin only, once)
  useEffect(() => {
    if (!isAdmin) return;
  ...
  // Load script performance (once on mount)
  useEffect(() => {
    import('../lib/supabase').then(({ default: supabase }) => {
      supabase.from('ai_scripts')
        ...
        .eq('is_active', true).gte('times_used', 1).order('times_used', { ascending: false }).limit(20)
        .then(({ data }) => setScriptPerf(data || []));
    });
  }, []);

  const { data, isLoading, error } = useAnalytics({ startISO, endISO, callerId });
  ...
  // Show loading while auth is loading or while waiting for caller to load (employee)
  if (authLoading || (!isAdmin && !caller?.id)) {
    return (
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '28px 32px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div>
            ...
            {!isAdmin && <p style={{ fontSize: 12, color: 'var(--text3)', margin: '2px 0 0' }}>{caller?.full_name || user?.email}</p>}
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          {isAdmin && (
            <select value={filterCaller || ''} onChange={e => setFilterCaller(e.target.value || null)}
              ...
              {allCallers.map(c => <option key={c.id} value={c.id}>{c.full_name}</option>)}
            </select>
          )}
      ...
      {isAdmin && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px 22px', marginBottom: 24 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>{['', 'Caller', 'Calls', 'Connected', 'Interested', 'Meetings', 'Rate', 'Avg Duration'].map(h => (
            ...
            <tbody>
              {(perCaller || []).length === 0 && (
                <tr><td colSpan={8} style={{ padding: '24px 12px', color: 'var(--text3)', fontSize: 13, textAlign: 'center' }}>No call data yet.</td></tr>
              )}
              {(perCaller || []).map((c, idx) => (
                <tr key={c.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '10px 12px', width: 28 }}><span style={{ fontSize: 14 }}>{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `#${idx + 1}`}</span></td>
                  <td style={{ padding: '10px 12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'white' }}>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    <span style={{ color: c.conversion_rate > 30 ? 'var(--green)' : c.conversion_rate > 10 ? 'var(--amber)' : 'var(--text3)' }}>{c.conversion_rate}%</span>
                  </td>
      ...
      {!isAdmin && myStats && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px 22px', marginBottom: 24 }}>
          <p style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.06em' }}>📊 Your Stats</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              ...
              { label: 'Avg Duration', value: myStats.avg_duration + 's', color: 'var(--text3)' },
            ].map(s => (
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px 22px' }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={(topCategories || []).slice(0, 8)} layout="vertical">
              ...
              <Tooltip contentStyle={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8 }} />
              <Bar dataKey="calls" fill="#6c63ff" name="Calls" radius={[0, 4, 4, 0]} />
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px 22px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 3 }}>
            {(hourlyPattern || []).filter(h => h.hour >= 8 && h.hour <= 19).map(h => {
              ...
              return (
                <div key={h.hour} title={`${h.hour}:00 — ${h.calls} calls`}
          ...
          <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 12 }}>Darker green = higher interest rate</p>
        </div>
      </div>
      ...
      {scriptPerf.length > 0 && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '20px 22px', marginTop: 24 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead><tr>{['Business', 'Country', 'Angle', 'Variant', 'Used', 'Converted', 'Rate', 'Rating'].map(h => (
                ...
                return (
                  <tr key={s.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '8px 10px' }}>
                      <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: s.variant === 'alternative' ? 'var(--amber-bg)' : 'var(--accent-glow)', color: s.variant === 'alternative' ? 'var(--amber)' : 'var(--accent2)' }}>{s.variant}</span>
                    </td>
